// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
	"github.com/go-openapi/validate"
)

// GetFlavorDetailOKBody get flavor detail o k body
//
// swagger:model getFlavorDetailOKBody
type GetFlavorDetailOKBody struct {

	// flavor
	// Required: true
	Flavor *Flavor `json:"flavor"`
}

// Validate validates this get flavor detail o k body
func (m *GetFlavorDetailOKBody) Validate(formats strfmt.Registry) error {
	var res []error

	if err := m.validateFlavor(formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (m *GetFlavorDetailOKBody) validateFlavor(formats strfmt.Registry) error {

	if err := validate.Required("flavor", "body", m.Flavor); err != nil {
		return err
	}

	if m.Flavor != nil {
		if err := m.Flavor.Validate(formats); err != nil {
			if ve, ok := err.(*errors.Validation); ok {
				return ve.ValidateName("flavor")
			}
			return err
		}
	}

	return nil
}

// ContextValidate validate this get flavor detail o k body based on the context it is used
func (m *GetFlavorDetailOKBody) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	var res []error

	if err := m.contextValidateFlavor(ctx, formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (m *GetFlavorDetailOKBody) contextValidateFlavor(ctx context.Context, formats strfmt.Registry) error {

	if m.Flavor != nil {
		if err := m.Flavor.ContextValidate(ctx, formats); err != nil {
			if ve, ok := err.(*errors.Validation); ok {
				return ve.ValidateName("flavor")
			}
			return err
		}
	}

	return nil
}

// MarshalBinary interface implementation
func (m *GetFlavorDetailOKBody) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *GetFlavorDetailOKBody) UnmarshalBinary(b []byte) error {
	var res GetFlavorDetailOKBody
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
