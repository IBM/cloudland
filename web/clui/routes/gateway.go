...
func (a *GatewayAdmin) Create(ctx context.Context, name string, pubID, priID int64, subnetIDs []int64) (gateway *model.Gateway, err error) {
	...
	pubIface, err := CreateInterface(ctx, pubSubnet.ID, gateway.ID, fmt.Sprintf("pub%d", pubSubnet.ID), "gateway_public", nil)
	if err != nil {
	...
	priIface, err := CreateInterface(ctx, priSubnet.ID, gateway.ID, fmt.Sprintf("pri%d", priSubnet.ID), "gateway_private", nil)
	if err != nil {
