...
func (a *SecruleAdmin) Create(secgroup int64, remoteIp, direction, protocol string, portMin, portMax int) (secrule *model.SecurityRule, err error) {
	...
	if err != nil {
		log.Println("DB failed to create security rule, %v", err)
		return
	}
	return
}

func (a *SecruleAdmin) Delete(id int64) (err error) {
	db := DB()
	db = db.Begin()
	defer func() {
		if err == nil {
	...
	if err = db.Delete(&model.SecurityRule{Model: model.Model{ID: id}}).Error; err != nil {
...
func (a *SecruleAdmin) List(offset, limit int64, order string) (total int64, secrules []*model.SecurityRule, err error) {
	db := DB()
	if limit == 0 {
		limit = 20
	}
	...
	if err = db.Model(&model.SecurityRule{}).Count(&total).Error; err != nil {
		log.Println("DB failed to count security rule(s), %v", err)
		return
	}
	db = dbs.Sortby(db.Offset(offset).Limit(limit), order)
	if err = db.Find(&secrules).Error; err != nil {
...
func (v *SecruleView) List(c *macaron.Context, store session.Store) {
	...
	total, secrules, err := secruleAdmin.List(offset, limit, order)
	if err != nil {
		log.Println("Failed to list security rule(s), %v", err)
...
func (v *SecruleView) Delete(c *macaron.Context, store session.Store) (err error) {
	...
	err = secruleAdmin.Delete(int64(secruleID))
	if err != nil {
...
func (v *SecruleView) Create(c *macaron.Context, store session.Store) {
	redirectTo := "../secrules"
	sgroupStr := c.Query("secgroup")
	remoteIp := c.Query("remoteip")
	secgroup, err := strconv.Atoi(sgroupStr)
	if err != nil {
		log.Println("Invalid security rule ID, %v", err)
		code := http.StatusBadRequest
		c.Error(code, http.StatusText(code))
	...
	_, err = secruleAdmin.Create(int64(secgroup), remoteIp, direction, protocol, portMax, portMin)
	if err != nil {
