...
func InstanceStatus(ctx context.Context, args []string) (status string, err error) {
	...
	if err != nil {
		logger.Debug("Failed to query hyper", err)
		return
	}
	...
	for i := 0; i < len(statusList); i += 2 {
		...
		if err != nil {
			...
			if gorm.IsRecordNotFoundError(err) {
				...
				if err != nil {
					logger.Debug("Failed to create unknown instance", err)
				}
			}
			continue
		}
		...
		if instance.Status != status {
			...
			if err != nil {
				logger.Debug("Failed to update status", err)
			}
		}
		if instance.Hyper != int32(hyperID) {
			...
			if err != nil {
				logger.Debug("Failed to hypervisor", err)
			}
			...
			if err != nil {
				logger.Debug("Failed to apply security groups", err)
				continue
			}
		}
	}
...
func ApplySecgroups(ctx context.Context, instance *model.Instance) (err error) {
	...
	if err = db.Set("gorm:auto_preload", true).Where("instance = ?", instance.ID).Find(&ifaces).Error; err != nil {
		logger.Debug("Interfaces query failed", err)
		return
	}
