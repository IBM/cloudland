...
func (a *FloatingIpAdmin) Create(ctx context.Context, instance *model.Instance, pubSubnet *model.Subnet, publicIp string, name string) (floatingIp *model.FloatingIp, err error) {
	...
	if pubSubnet != nil && pubSubnet.Type != "public" {
		logger.Debug("Subnet must be public", err)
	...
	if err != nil {
		logger.Debug("DB failed to allocate floating ip", err)
		return
	}
	...
	if err != nil {
		logger.Debug("DB failed to update floating ip", err)
		return
	}
	return
}
...
func (a *FloatingIpAdmin) Attach(ctx context.Context, floatingIp *model.FloatingIp, instance *model.Instance) (err error) {
	...
	if instance == nil || instance.Status != "running" {
		logger.Debug("Instance is not running")
	...
	if routerID == 0 {
		logger.Debug("Instance has no router")
...
func (a *FloatingIpAdmin) Get(ctx context.Context, id int64) (floatingIp *model.FloatingIp, err error) {
	...
	if err != nil {
		logger.Debug("DB failed to query floatingIp ", err)
		return
	}
...
func (a *FloatingIpAdmin) GetFloatingIpByUUID(ctx context.Context, uuID string) (floatingIp *model.FloatingIp, err error) {
	...
	if err != nil {
		logger.Debug("Failed to query floatingIp, %v", err)
		return
	}
	if floatingIp.InstanceID > 0 {
		floatingIp.Instance = &model.Instance{Model: model.Model{ID: floatingIp.InstanceID}}
...
func (a *FloatingIpAdmin) Detach(ctx context.Context, floatingIp *model.FloatingIp) (err error) {
	...
	if floatingIp.Instance != nil {
		...
		err = hyperExecute(ctx, control, command)
		if err != nil {
			logger.Debug("Detach floating ip failed", err)
			return
		}
	}
	...
	if err != nil {
		logger.Debug("Failed to update instance ID for floating ip", err)
		return
	}
	return
}
...
func (a *FloatingIpAdmin) Delete(ctx context.Context, floatingIp *model.FloatingIp) (err error) {
	...
	if floatingIp.Instance != nil {
		...
		if err != nil {
			logger.Debug("Failed to detach floating ip", err)
			return
		}
	}
	err = DeallocateFloatingIp(ctx, floatingIp.ID)
	if err != nil {
		logger.Debug("DB failed to deallocate floating ip", err)
		return
	}
	return
}
...
func (v *FloatingIpView) List(c *macaron.Context, store session.Store) {
	...
	if err != nil {
		logger.Debug("Failed to list floating ip(s), %v", err)
...
func (v *FloatingIpView) Delete(c *macaron.Context, store session.Store) (err error) {
	...
	if err != nil {
		logger.Debug("Invalid floating ip ID ", err)
		c.Data["ErrorMsg"] = err.Error()
		c.HTML(http.StatusBadRequest, "error")
		return
	}
	...
	if err != nil {
		logger.Debug("Failed to delete floating ip, %v", err)
		c.Data["ErrorMsg"] = err.Error()
		c.HTML(http.StatusBadRequest, "error")
		return
	}
...
func (v *FloatingIpView) New(c *macaron.Context, store session.Store) {
	...
	if err != nil {
		logger.Debug("Failed to query instances %v", err)
		return
	}
