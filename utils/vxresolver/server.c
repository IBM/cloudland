...
struct vlan_peer vlan_peer_head = {{0}, NULL};
...
{
	struct vlan_peer *vp = NULL;

	if (strncasecmp(command, "add", 3) == 0) {
		struct vlan_peer *peer = NULL;
        char *pip = NULL;
        ...
        pip += 1;
		for (vp = vlan_peer_head.next; vp != NULL; vp = vp->next) {
			if (strcmp(vp->addr, pip) == 0) {
                return 0;
			}
		}

		peer = (struct vlan_peer *)malloc(sizeof(struct vlan_peer));
		memset(peer, 0, sizeof(struct vlan_peer));
        strncpy(peer->addr, pip, sizeof(peer->addr));
		peer->next = vlan_peer_head.next;
		vlan_peer_head.next = peer;
		printf("Add peer %s", peer->addr);
	} else if (strncasecmp(command, "del", 3) == 0) {
        char *pip = command + 4;
		struct vlan_peer *vprev = &vlan_peer_head;
		for (vp = vlan_peer_head.next; vp != NULL; vp = vp->next) {
			if (strcmp(vp->addr, pip) == 0) {
				break;
			}
