...
action_target $hyper "sudo timeout_virsh define $xml_dir/${vm_ID}.xml && sudo timeout_virsh start $vm_ID"
state=$(action_target $hyper "sudo timeout_virsh dominfo $vm_ID" | grep State | cut -d: -f2- | xargs)
