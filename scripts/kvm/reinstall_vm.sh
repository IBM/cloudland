...
if [ -z "$wds_address" ]; then
    ...
    if [ ! -f "$vm_img" ]; then
        ...
        if [ ! -s "$image_cache/$img_name" ]; then
            ...
            echo "|:-COMMAND-:| `basename $0` '$ID' '$state' '$SCI_CLIENT_ID' '$vol_ID' 'image $img_name not available!'"
            exit -1
        fi
    ...
    if [ -z "$snapshot_id" -o "$snapshot_id" = null ]; then
        ...
        snapshot_id=$(wds_curl GET "api/v2/sync/block/snaps?name=$snapshot_name" | jq -r '.snaps[0].id')
        if [ -z "$snapshot_id" -o "$snapshot_id" = null ]; then
            echo "|:-COMMAND-:| `basename $0` '$ID' '$state' '$SCI_CLIENT_ID' '$vol_ID' 'failed to create image snapshot, $snapshot_ret'"
            exit -1
        fi
    ...
    if [ -z "$volume_id" -o "$volume_id" = null ]; then
        echo "|:-COMMAND-:| `basename $0` '$ID' '$state' '$SCI_CLIENT_ID' '$vol_ID' 'failed to create boot volume based on snapshot $snapshot_name, $volume_ret!'"
        exit -1
    fi
    expand_ret=$(wds_curl PUT "api/v2/sync/block/volumes/$volume_id/expand" "{\"size\": $fsize}")
    ret_code=$(echo $expand_ret | jq -r .ret_code)
    if [ "$ret_code" != "0" ]; then
        echo "|:-COMMAND-:| `basename $0` '$ID' '$state' '$SCI_CLIENT_ID' '$vol_ID' 'failed to expand boot volume to size $fsize, $expand_ret'"
        exit -1
    fi
    ...
    if [ "$ret_code" != "0" ]; then
        echo "|:-COMMAND-:| `basename $0` '$ID' '$state' '$SCI_CLIENT_ID' '$vol_ID' 'failed to create wds vhost for boot volume, $vhost_ret, $uss_ret!'"
        exit -1
    fi
