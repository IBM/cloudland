...
i=0
while [ $i -lt $nvlan ]; do
    read -d'\n' -r vlan ip mac gateway router inbound outbound allow_spoofing < <(jq -r ".[$i].vlan, .[$i].ip_address, .[$i].mac_address, .[$i].gateway, .[$i].router, .[$i].inbound, .[$i].allow_spoofing" <<<$vlans)
